...
**Goal:** Add a manual `prompt-architect` skill that turns unclear, broken, incomplete, or complex user intent into clear prompts, system prompts, or reusable prompt packages.

**Architecture:** Add one new source skill under `skills/prompt-architect/` using Aphelocoma's existing `metadata.yaml` plus `skill.md` format. Keep the first version self-contained because this is a reasoning workflow and does not need scripts, templates, references, or adapter overrides.

**Tech Stack:** Markdown, YAML, Bash validation through `bin/aph`.

---

## File Structure

- Create: `skills/prompt-architect/metadata.yaml`
  - Responsibility: Declare the skill name, trigger description, and manual invocation type.
- Create: `skills/prompt-architect/skill.md`
  - Responsibility: Define the Prompt Architect workflow, clarification gate, domain-agnostic behavior, output formats, prompt package behavior, and quality rules.
- Do not modify: `adapters/`, `bin/aph`, or existing skills.
  - Reason: The existing skill discovery and Codex deploy path already support source skills with `metadata.yaml` and `skill.md`.

## Important Local State

- There is an unrelated untracked `AGENTS.md` in the repo root. Do not stage, edit, remove, or commit it.

### Task 1: Add Skill Metadata

**Files:**
- Create: `skills/prompt-architect/metadata.yaml`

- [ ] **Step 1: Verify the skill is not currently discoverable**

Run:

```bash
./bin/aph skills | rg 'prompt-architect'
```

Expected: command exits with status 1 and prints no output.
...
```diff
*** Begin Patch
*** Add File: skills/prompt-architect/metadata.yaml
...
```bash
sed -n '1,20p' skills/prompt-architect/metadata.yaml
```
...
- [ ] **Step 4: Verify the skill appears in the CLI listing**
...
- [ ] **Step 1: Verify the instruction file does not exist yet**

Run:

```bash
test ! -f skills/prompt-architect/skill.md
```
...
```diff
*** Begin Patch
*** Add File: skills/prompt-architect/skill.md
...
```bash
rg -n "Core Mission|Classify the Request|Infer the Domain|Output Formats|Domain-Specific Quality Checks|Guardrails|Quality Bar" skills/prompt-architect/skill.md
```

Expected output includes one line for each heading:
...
### Task 3: Validate Discovery and Deployment

**Files:**
- Verify: `skills/prompt-architect/metadata.yaml`
- Verify: `skills/prompt-architect/skill.md`
- Verify generated temp file: `.codex/skills/prompt-architect/SKILL.md` inside a temporary `HOME`
...
```bash
rg -n "Do not answer the user's underlying task|Ask clarifying questions only when|Never force software-development framing|Reusable Prompt Package|neuroscience educator" skills/prompt-architect/skill.md
```

Expected:

- Matches in `skill.md` for the underlying-task guardrail.
- Matches in `skill.md` for the clarification threshold.
- Matches in `skill.md` for the domain-agnostic rule.
- Matches in `skill.md` for the reusable prompt package.
- Matches in `skill.md` for the non-software neuroscience example.

Run:

```bash
! rg -n "disable-model-invocation" skills/prompt-architect/metadata.yaml skills/prompt-architect/skill.md
```

Expected: no output and exit status 0. `disable-model-invocation` must not be written in the source files; Codex deploy generates it from `type: manual`.

- [ ] **Step 2: Verify CLI discovery**
...
- [ ] **Step 3: Verify Codex deploy generates manual-skill frontmatter**
...
trap 'rm -rf "$tmp"' EXIT
mkdir -p "$tmp/home"
HOME="$tmp/home" ./bin/aph setup >/dev/null
HOME="$tmp/home" ./bin/aph deploy codex >/dev/null
sed -n '1,24p' "$tmp/home/.codex/skills/prompt-architect/SKILL.md"
rg -n "name: prompt-architect|disable-model-invocation: true|You are Prompt Architect" "$tmp/home/.codex/skills/prompt-architect/SKILL.md"
```
...
Expected `rg` output includes matches for:

```text
name: prompt-architect
disable-model-invocation: true
You are Prompt Architect
```

### Task 4: Final Review and Commit

**Files:**
- Stage: `skills/prompt-architect/metadata.yaml`
- Stage: `skills/prompt-architect/skill.md`
- Do not stage: `AGENTS.md`

- [ ] **Step 1: Check formatting and accidental whitespace issues**

Run:

```bash
git diff --check -- skills/prompt-architect/metadata.yaml skills/prompt-architect/skill.md
```
...
```bash
git diff -- skills/prompt-architect/metadata.yaml skills/prompt-architect/skill.md
```

Expected: diff only creates the two prompt-architect skill files.

- [ ] **Step 3: Confirm untracked unrelated files are not staged**
...
If `AGENTS.md` appears as staged, unstage it with:

```bash
git restore --staged AGENTS.md
```

- [ ] **Step 4: Commit only the new skill files**

Run:

```bash
git add skills/prompt-architect/metadata.yaml skills/prompt-architect/skill.md
git commit -m "Add prompt architect skill"
```

Expected: commit succeeds and reports two new files.

- [ ] **Step 5: Report completion and offer Aphelocoma follow-ups**
...
- the commit hash
- a note that `AGENTS.md` was left untouched
- an offer to create an ADR in `core/projects/aphelocoma/adrs/`
- an offer to update `core/projects/aphelocoma/tasks.md`
